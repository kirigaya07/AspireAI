...
import { calculateOpenAITokenUsage } from "@/lib/openai-tokens";
...
// Function to track OpenAI model token usage
export async function trackOpenAIUsage(
  ...
  try {
    ...
    // Calculate tokens used by OpenAI
    const tokenUsage = calculateOpenAITokenUsage(input, output, model);
    ...
    console.error(`Error tracking OpenAI usage: ${error.message}`);
