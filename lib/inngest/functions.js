...
import { getPackageById, TOKEN_PACKAGES } from "@/lib/constants";
...
// ─────────────────────────────────────────────────────────────────
// Payment reconciliation: catches stuck PENDING payments where
// both verify and webhook failed. Runs every 30 minutes.
// ─────────────────────────────────────────────────────────────────
export const reconcilePendingPayments = inngest.createFunction(
  { id: "reconcile-pending-payments", name: "Reconcile Pending Razorpay Payments" },
  { cron: "*/30 * * * *" },
  async ({ step }) => {
    const stuckPayments = await step.run("Find stuck pending payments", async () => {
    ...
    for (const payment of stuckPayments) {
      await step.run(`Reconcile ${payment.razorpayId}`, async () => {
        try {
          const rzpOrder = await razorpay.orders.fetch(payment.razorpayId);

          if (rzpOrder.status === "paid") {
            const payments = await razorpay.orders.fetchPayments(payment.razorpayId);
            ...
            const tokenPackage =
              (notes.packageId && getPackageById(notes.packageId)) ||
              TOKEN_PACKAGES.find((p) => Math.abs(p.amount - payment.amount) < 0.01);

            if (!tokenPackage) {
              console.error("[reconcile] Cannot resolve package for", payment.razorpayId);
              skipped++;
              return;
            }
            ...
            reconciled++;
            console.log(`[reconcile] Recovered payment ${payment.razorpayId}`);
          } else if (
            rzpOrder.status === "attempted" &&
            payment.createdAt < new Date(Date.now() - 2 * 60 * 60 * 1000)
          ) {
          ...
          console.error(`[reconcile] Error on ${payment.razorpayId}:`, err.message);
          skipped++;
        }
      });
    }

    return { reconciled, failed, skipped, total: stuckPayments.length };
  }
);
