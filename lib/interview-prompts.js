...
/**
 * Builds the prompt to compare interview weaknesses against the user's resume.
 */
export function buildGapAnalysisPrompt({ improvements, jobTitle, type, resumeContent }) {
  return `You are a career coach analyzing gaps between a candidate's interview weaknesses and their resume.

INTERVIEW WEAKNESSES (from a ${type.replace("_", " ")} interview for ${jobTitle}):
${improvements.map((item, idx) => `${idx + 1}. ${item}`).join("\n")}

RESUME CONTENT:
${resumeContent}

Identify specific gaps between what the interview revealed as weaknesses and what is missing or underrepresented in the resume. Return ONLY valid JSON (no markdown, no backticks) in this exact format:
{
  "missingSkills": ["<skill 1>", "<skill 2>", "<skill 3>"],
  "projectIdeas": ["<concrete project to build that addresses the gaps>", "<another project idea>"],
  "resumeTweaks": ["<specific resume change 1>", "<specific resume change 2>", "<specific resume change 3>"],
  "priorityAction": "<the single most impactful change to make right now>"
}

Be specific. Reference actual content from the resume and interview weaknesses. Do not repeat generic advice.`;
}
